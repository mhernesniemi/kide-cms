...
| `json`     | `schema?: string`; `itemFields?: Record<string, FieldConfig>` — with `admin.component: "repeater"`, renders a typed repeater (array of objects) whose rows use these named sub-fields (text/select/boolean/image/richText/link — link gets the internal-link picker) instead of raw JSON; works both as a block sub-field and as a top-level collection field |
