...
export const cms = createCms(config, access, hooks) as ReturnType<typeof createCms> & {
  users: {
    find(options?: import("../core/api").FindOptions, context?: { user?: { id: string; role?: string; email?: string } | null }): Promise<UsersDocument[]>;
    findOne(filter: Record<string, unknown> & { locale?: string; status?: "draft" | "published" | "any" }, context?: { user?: { id: string; role?: string; email?: string } | null }): Promise<UsersDocument | null>;
    findById(id: string, options?: { locale?: string; status?: "draft" | "published" | "any" }, context?: { user?: { id: string; role?: string; email?: string } | null }): Promise<UsersDocument | null>;
