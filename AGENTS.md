...
- **API method names:** `cms.<collection>.find / findOne / findById / create / createMany / upsert / update / delete / count / publish / unpublish / schedule / versions / restore / getTranslations / upsertTranslation`. There is no `findMany` (that's Prisma/Payload) — it fails at runtime with "is not a function"; `pnpm check` catches it at the type level.
